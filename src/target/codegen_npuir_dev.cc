...
void CodeGenTileLangNPUIRDEV::VisitStmt_(const AttrStmtNode *op) {
  if (op->attr_key == "thread_extent") {
    ...
    if (iv->thread_tag == "blockIdx.x" && iv->var->name_hint != "_") {
      ...
      int y_val = extent_y.as<IntImmNode>()->value;
      auto y_const = builder.create<mlir::arith::ConstantOp>(
          mlir::UnknownLoc::get(&context), builder.getI32Type(),
          builder.getI32IntegerAttr(y_val));
      ...
      auto forOp = builder.create<mlir::scf::ForOp>(
          mlir::UnknownLoc::get(&context), lowerBound, y_const, step);
